...
fn make_payload(id: u32, asn: u16) -> Vec<u8> {
    ...
    buf.push(0xa2); buf.extend_from_slice(b"id");
    buf.push(0xce); buf.extend_from_slice(&id.to_be_bytes());
    buf.push(0xa3); buf.extend_from_slice(b"asn");
    buf.push(0xcd); buf.extend_from_slice(&asn.to_be_bytes());
    buf.push(0xa2); buf.extend_from_slice(b"cc");
    buf.push(0xa2); buf.extend_from_slice(b"US");
    buf
}
...
fn make_db(n_objects: usize) -> NamedTempFile {
    ...
    w.build(tmp.path().to_str().expect("utf-8 path")).expect("build");
    tmp
}
...
fn bench_lookup(c: &mut Criterion) {
    ...
    let db = ReaderCore::open(tmp.path().to_str().expect("utf-8 path"), false)
        .expect("open");
    ...
    g.bench_function("exists/hit",  |b| b.iter(|| black_box(db.exists(b"10.1.1.1"))));
    g.bench_function("exists/miss", |b| b.iter(|| black_box(db.exists(b"192.168.1.1"))));
    g.bench_function("prefix/scan", |b| b.iter(|| black_box(db.prefix(b"10.10."))));
