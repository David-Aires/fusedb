...
class TestIntegrity:
    ...
    def test_verify_raises_on_corrupt(self, tmp):
        ...
        with open(p, "r+b") as f:
            f.seek(50); f.write(b"\x00\x00\x00\x00")
...
class TestConcurrency:
    ...
    def test_reloadable_concurrent(self, tmp):
        p = str(tmp / "live.fsdb")
        w = FuseWriter()
        for i in range(100): w.add(f"k{i}", {"v": i})
        w.build(p)
        ...
        def reader():
            for _ in range(500):
                v = db.get("k0")
                if v is None: errors.append("none")
        ...
        time.sleep(0.02)
        w2 = FuseWriter()
        for i in range(100): w2.add(f"k{i}", {"v": i + 100})
        w2.build(p)
        time.sleep(0.01)
        db.reload()
...
class TestMerge:
    def test_merge_two_files(self, tmp):
        ...
        wa = FuseWriter(); wa.add("key_a", {"src": "A"}); wa.build(pa)
        wb = FuseWriter(); wb.add("key_b", {"src": "B"}); wb.build(pb)
    ...
    def test_merge_deduplicates(self, tmp):
        ...
        obj = {"org": "Google"}
        wa  = FuseWriter(); wa.add("8.8.8.8", obj); wa.build(pa)
        wb  = FuseWriter(); wb.add("gmail.com", obj); wb.build(pb)
        merge(pa, pb, output=pm)
...
class TestWatcher:
    def test_watcher_auto_reloads(self, tmp):
        ...
        w = FuseWriter(); w.add("k", {"v": 1}); w.build(p)
        ...
        time.sleep(0.05)
        # rebuild the file
        w2 = FuseWriter(); w2.add("k", {"v": 2}); w2.build(p)
...
class TestPool:
    ...
    def test_pool_swap(self, tmp, simple_db):
        p2 = str(tmp / "v2.fsdb")
        w2 = FuseWriter(); w2.add("new_key", {"v": 2}); w2.build(p2)
    ...
    def test_pool_concurrent_swap(self, tmp):
        ...
        p2 = str(tmp / "p2.fsdb")
        w1 = FuseWriter()
        for i in range(200): w1.add(f"k{i}", {"v": 1})
        w1.build(p1)
        w2 = FuseWriter()
        for i in range(200): w2.add(f"k{i}", {"v": 2})
        w2.build(p2)
